...
m_MF=(1-x_MF)
a=1+lambertw(-m_MF*numpy.exp(-m_MF-x_MF*1e-3))/m_MF

tau_max_m_h = -1/(numpy.log(m_MF)-m_MF*a - x_MF*1e-3)
...
for i in [1,2,3,4,5,6,7]:
    ...
    if i>5:
        ax1.plot(x_MF,tau_max_m_h,'black',lw=0.5, ls='dashed')
    if i>6:
        for j, k in enumerate([1e4,1e3,1e2,25]):
            x_NE   = x_MF
    ...
    if i==7:
        plt.savefig("fig_ER_tau.png", transparent=True, dpi=300)
    if i==6:
        plt.savefig("fig_ER_tau_correction.png", transparent=True, dpi=300)
    if i==5:
        plt.savefig("fig_ER_tau_data3.png", transparent=True, dpi=300)
    if i==3:
        plt.savefig("fig_ER_tau_data2.png", transparent=True, dpi=300)
    if i==2:
        plt.savefig("fig_ER_tau_data1.png", transparent=True, dpi=300)
    if i==1:
        plt.savefig("fig_ER_tau_empty.png", transparent=True, dpi=300)
