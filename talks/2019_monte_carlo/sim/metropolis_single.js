...
function metropolis_sampling_ergodic(ID,func_y,func_Px,xmin,xmax,ymin,ymax){
  ...
  function generate_points(){
    //remove old dots
    clear_dot(ctx,axes,func_y, func_Px, x,y,radius);
    //update
    if(Math.random()<0.9){
      dx = -0.1+0.2*Math.random();
    }
    else{
      dx = -0.5+1.0*Math.random();
    }
    if(x+dx>0 && x+dx<1){
      Px_old = func_Px(x);
      x += dx;
      Px_new = func_Px(x);
      P_accept = Px_new/Px_old;
        //accept or not
      if(P_accept < 1){
...
function metropolis_sampling(ID,func_y,func_Px,xmin,xmax,ymin,ymax){
  var canvas = document.getElementById(ID);
  if (null==canvas || !canvas.getContext) return;
  canvas.width  = canvas.clientWidth;
  canvas.height = canvas.clientHeight;
